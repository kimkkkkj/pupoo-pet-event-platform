...
const styles = `
  /* ── Outer ── */
  .signup-outer { background: #f5f7f2; min-height: 100vh; }
  .signup-inner { padding: calc(var(--pupoo-site-header-offset, 92px) + 44px) 24px 96px; }
  .signup-wrap {
  ...
  .signup-title {
    ...
    text-align: center; margin: 0 0 10px; letter-spacing: -0.03em;
  }
  .signup-desc {
    font-size: 15.5px; color: #6b7280; text-align: center;
    margin: 0 0 36px; line-height: 1.6;
  }

  /* ── Step ── */
  .step-bar { display: flex; align-items: center; justify-content: center; margin-bottom: 36px; }
  .step-item { display: flex; flex-direction: column; align-items: center; gap: 10px; min-width: 84px; }
  .step-num {
    width: 46px; height: 46px; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    background: #fff; color: #c0c5cc; transition: all 0.3s;
    border: 1.5px solid #e5e7eb;
  }
  .step-num.active { background: #6FA436; color: #fff; border-color: #6FA436; box-shadow: 0 0 0 5px rgba(144, 196, 80, 0.2); }
  .step-num.done   { background: #e3f0d2; color: #4d7a1f; border-color: #e3f0d2; }
  .step-txt { font-size: 13.5px; color: #9ca3af; font-weight: 600; transition: color 0.3s; }
  .step-txt.active { color: #111827; font-weight: 800; }
  .step-txt.done   { color: #5E8F2A; }
  .step-line {
    width: 56px; height: 2px; background: #e5e7eb; border-radius: 2px;
    margin: 0 4px 30px; transition: background 0.3s;
  }
  .step-line.done { background: #90C450; }

  /* ── Card ── */
  .card-panel {
    background: #fff; border-radius: 24px; padding: 40px 44px; margin-bottom: 20px;
    box-shadow: 0 20px 60px rgba(40, 60, 20, 0.08), 0 4px 14px rgba(0, 0, 0, 0.03);
  }

  /* ── Section ── */
  .section-divider { border: none; border-top: 1px solid #eceef1; margin: 32px 0; }
  .section-label {
  ...
  .field-label .req { color: #6FA436; font-size: 14px; }
  .field-body { width: 100%; min-width: 0; }
  .field-helper { margin-top: 10px; font-size: 13px; color: #9ca3af; line-height: 1.5; }

  /* ── Hint banner ── */
  .hint-banner {
    display: flex; align-items: center; gap: 8px;
    padding: 10px 14px; border-radius: 12px;
    border: 1px solid #e8e6f0; font-size: 13px;
  ...
  .fi {
    ...
    padding: 0 16px; font-size: 16px; color: #111827;
    outline: none; background: #fafbfa;
    transition: border-color 0.2s, box-shadow 0.2s, background 0.2s; font-family: inherit;
    box-sizing: border-box;
  }
  .fi:focus { border-color: #90C450; background: #fff; box-shadow: 0 0 0 4px rgba(144, 196, 80, 0.16); }
  .fi::placeholder { color: #b0b6bf; font-size: 15px; }
  .fi:disabled { background: #f3f4f6; color: #9ca3af; }

  /* ── Custom Select ── */
  .cselect-wrap { position: relative; display: inline-block; }
  .cselect-trigger {
    display: flex; align-items: center; justify-content: space-between;
    gap: 10px; height: 54px; min-width: 110px; padding: 0 16px;
    border: 1.5px solid #e5e7eb; border-radius: 14px;
    background: #fafbfa; cursor: pointer; font-family: inherit;
    font-size: 15px; font-weight: 600; color: #111827;
    outline: none; transition: all 0.15s; box-sizing: border-box;
  }
  .cselect-trigger:hover { border-color: #d1d5db; }
  .cselect-trigger.open { border-color: #90C450; background: #fff; box-shadow: 0 0 0 4px rgba(144, 196, 80, 0.16); }
  .cselect-trigger:disabled { background: #f3f4f6; color: #9ca3af; cursor: not-allowed; }
  ...
  .cselect-menu {
    ...
    padding: 6px; box-shadow: 0 10px 30px rgba(0,0,0,.12); box-sizing: border-box;
  ...
  .cselect-option {
    ...
    border: none; background: none; color: #4b5563;
    font-size: 14.5px; font-weight: 500; cursor: pointer;
    text-align: left; font-family: inherit; transition: background 0.1s;
  }
  .cselect-option:hover { background: #f4f8ee; }
  .cselect-option.active { color: #4d7a1f; font-weight: 800; background: #f4f8ee; }

  /* ── Side button (중복확인 등) ── */
  .btn-addr {
    height: 54px; padding: 0 22px; background: #fff; color: #5E8F2A;
    border: 1.5px solid #90C450; border-radius: 14px; font-size: 15px; font-weight: 700;
    cursor: pointer; white-space: nowrap; font-family: inherit;
    transition: background 0.15s; flex-shrink: 0;
  }
  .btn-addr:hover { background: #f4f8ee; }
  .btn-addr:disabled { color: #9ca3af; border-color: #e5e7eb; background: #f3f4f6; cursor: not-allowed; }

  /* ── Pet ── */
  .pet-card {
    background: #fafbfa; border: 1.5px solid #eef0ea;
  ...
  .pet-grid .fi { height: 50px; font-size: 15px; background: #fff; }
  .pet-grid .cselect-trigger { width: 100%; height: 50px; font-size: 14.5px; min-width: 100px; background: #fff; }
  .btn-pet-circle {
    width: 34px; height: 34px; border: none; border-radius: 50%;
    cursor: pointer; display: flex; align-items: center; justify-content: center;
    transition: background 0.15s; color: #fff;
  }
  .btn-pet-add { background: #6FA436; }
  .btn-pet-add:hover { background: #5E8F2A; }
  .btn-pet-del { background: #d1d5db; }
  .btn-pet-del:hover { background: #9ca3af; }
  .btn-pet-del:disabled { opacity: 0.4; cursor: not-allowed; }

  /* ── Verify ── */
  .verify-section { text-align: center; padding: 24px 0 8px; }
  .verify-icon-wrap {
    width: 72px; height: 72px; border-radius: 50%;
    background: #eef6e4; display: flex; align-items: center;
  ...
  .otp-digit-box {
    width: 58px; height: 66px; border: 1.5px solid #e5e7eb;
    border-radius: 14px; background: #fafbfa;
    text-align: center; font-size: 28px; font-weight: 800;
    color: #111827; caret-color: #6FA436;
    outline: none; transition: border-color .15s, box-shadow .15s;
  }
  .otp-digit-box:focus {
    border-color: #90C450; box-shadow: 0 0 0 4px rgba(144, 196, 80, 0.16);
    background: #fff;
  }
  ...
  .btn-primary {
    flex: 1; height: 58px; padding: 0 24px;
    background: #6FA436; color: #fff; border: none; border-radius: 14px;
    font-size: 17px; font-weight: 800; cursor: pointer;
    transition: background 0.15s, box-shadow 0.15s; font-family: inherit;
    display: flex; align-items: center; justify-content: center; gap: 8px;
    box-sizing: border-box; box-shadow: 0 10px 24px rgba(111, 164, 54, 0.26);
  }
  .btn-primary:hover { background: #5E8F2A; }
  .btn-primary:disabled { background: #e5e7eb; color: #9ca3af; box-shadow: none; cursor: not-allowed; }
  .btn-outline {
    flex: 0 0 34%; height: 58px; padding: 0 24px;
  ...
  .btn-text-link {
    ...
    cursor: pointer; text-decoration: underline; text-underline-offset: 3px; padding: 0; font-family: inherit;
  }
  ...
  .notice-box {
    ...
    background: #f4f8ee; border: 1px solid #e3f0d2;
    border-radius: 18px; padding: 18px 22px;
    margin-bottom: 20px; font-size: 14.5px; color: #4b5563; line-height: 1.75;
  }
  .notice-box b { color: #111827; font-weight: 700; }

  /* ── Terms ── */
  .terms-all {
    display: flex; align-items: center; gap: 14px;
    padding: 20px 22px; background: #fafbfa; border: 1.5px solid #e5e7eb;
    border-radius: 16px; margin-bottom: 12px; cursor: pointer;
    transition: border-color 0.15s, background 0.15s;
  }
  .terms-all:hover { border-color: #c9ddb0; }
  .terms-all.checked { background: #f4f8ee; border-color: #90C450; }
  .terms-all label { font-size: 17px; font-weight: 800; color: #111827; cursor: pointer; }
  .terms-item { padding: 16px 4px; border-bottom: 1px solid #f1f3f5; }
  .terms-item:last-child { border-bottom: none; }
  .terms-item-head { display: flex; align-items: center; gap: 12px; cursor: pointer; }
  .terms-item-head label { font-size: 15.5px; font-weight: 500; color: #374151; cursor: pointer; flex: 1; }
  .terms-item-head .terms-tag { font-size: 14px; font-weight: 700; color: #5E8F2A; }
  .terms-item-head .terms-tag-opt { font-size: 14px; font-weight: 500; color: #9ca3af; }
  .btn-terms-view {
  ...
  .btn-terms-view:hover { color: #111827; border-color: #d1d5db; }
  .terms-box {
    background: #fafbfa; border: 1px solid #eef0ea; border-radius: 12px;
    padding: 20px 22px; max-height: 200px; overflow-y: auto;
    font-size: 13px; color: #555; line-height: 1.9; margin-top: 12px;
  }
  ...
  .terms-checkbox:checked {
    border-color: #6FA436; background: #6FA436;
  ...
  .terms-helper { margin: 16px 0 0; padding: 0 4px; font-size: 12.5px; color: #9ca3af; line-height: 1.8; }
  ...
  @media (max-width: 900px) {
    .card-panel { padding: 32px 26px; border-radius: 20px; }
  ...
  @media (max-width: 640px) {
    .signup-inner { padding: calc(var(--pupoo-site-header-offset, 72px) + 20px) 14px 48px; }
    .signup-title { font-size: 26px; }
    .signup-desc { font-size: 14px; margin-bottom: 26px; }
    .card-panel { padding: 22px 18px; border-radius: 18px; margin-bottom: 14px; }
    .step-bar { margin-bottom: 26px; }
    .step-item { min-width: 58px; gap: 8px; }
    .step-num { width: 36px; height: 36px; }
    .step-txt { font-size: 11px; }
    .step-line { width: 16px; margin-bottom: 26px; }
    .section-label { font-size: 17px; margin-bottom: 12px; padding-bottom: 12px; }
    .field { padding: 10px 0; }
    .fi { height: 50px; font-size: 15px; padding: 0 14px; }
    .cselect-trigger { height: 50px; font-size: 14px; min-width: 88px; }
    .btn-addr { height: 50px; padding: 0 16px; font-size: 14px; }
    .btn-row { flex-direction: column-reverse; gap: 10px; margin-top: 20px; }
    .btn-primary, .btn-outline { flex: none; width: 100%; height: 54px; font-size: 16px; }
    .hint-banner { font-size: 12.5px; padding: 8px 12px; }
    .notice-box { padding: 14px 16px; font-size: 13.5px; }
    .terms-all { padding: 16px 16px; }
    .terms-all label { font-size: 15px; }
    .terms-item-head label { font-size: 14px; }
    .btn-terms-view { padding: 6px 11px; font-size: 12px; }
    .pet-card { padding: 16px 14px; }
    .verify-section { padding: 12px 0 4px; }
    .otp-digit-row { gap: 6px; margin: 20px auto 20px; max-width: 100%; }
    .otp-digit-box { width: calc((100% - 30px) / 6); height: 56px; font-size: 22px; border-radius: 12px; }
...
// 영문 + 숫자 + 특수문자 조합 8~16자
const PASSWORD_RULE = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[^A-Za-z0-9\s]).{8,16}$/;
...
export default function JoinNormal() {
  ...
  const [form, setForm] = useState({
    ...
    email: "", mobile1: "010", mobile2: "", mobile3: "",
  });
  ...
  const signupStart = async () => {
    ...
    if (!isSocial) {
      ...
      if (!PASSWORD_RULE.test(form.password)) throw new Error("비밀번호는 영문, 숫자, 특수문자를 섞어 8~16자로 입력해 주세요.");
  ...
  const requestEmailVerification = async () => {
    ...
    try {
      ...
      const devCode = res?.devToken ?? res?.data?.devToken;
      setEmailCode(devCode ? String(devCode) : "");
      setEmailRequested(true);
      setEmailRequestMessage(
        devCode
          ? "개발 환경이라 메일 대신 인증 코드를 자동으로 입력했어요."
          : "이메일을 확인해 주세요. 받은 인증 코드를 입력하면 가입을 완료할 수 있습니다.",
      );
        ...
        {step === "AGREE" && (
          <>
            <div className="card-panel">
              ...
              <div className={`terms-all${agreeAll ? " checked" : ""}`} onClick={() => handleAgreeAll(!agreeAll)}>
              ...
              {!isSocial && (
                <>
                  <div className="field">
                    ...
                    <div className="field-body">
                      <input className="fi" type="password" name="password" value={form.password} onChange={handleFormChange} placeholder="영문/숫자/특수문자 조합 8~16자" autoComplete="new-password" disabled={loading} />
                      {form.password && !PASSWORD_RULE.test(form.password) && (
                        <HintBanner icon={AlertCircle} variant="warn">영문, 숫자, 특수문자를 섞어 8~16자로 입력해 주세요</HintBanner>
                      )}
                    </div>
                  </div>
                  <div className="field">

...
@Entity
@Table(
        name = "ai_event_congestion_timeseries",
        uniqueConstraints = {
                @UniqueConstraint(
                        name = "uk_ai_event_congestion_event_time",
                        columnNames = {"event_id", "timestamp_minute"}
                )
        },
        indexes = {
                @Index(name = "ix_ai_event_congestion_time", columnList = "timestamp_minute"),
                @Index(name = "ix_ai_event_congestion_event_time", columnList = "event_id,timestamp_minute")
        }
)
@Getter
...
@Builder
public class AiEventCongestionTimeseries {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "event_timeseries_id", nullable = false)
    private Long eventTimeseriesId;
    ...
    @Column(name = "total_wait_count", nullable = false)
    private Integer totalWaitCount;

    @Column(name = "avg_wait_min", precision = 10, scale = 2)
    private BigDecimal avgWaitMin;

    @Column(name = "running_program_count", nullable = false)
    private Integer runningProgramCount;
    ...
    public void applySnapshot(
        ...
        this.runningProgramCount = runningProgramCount;
